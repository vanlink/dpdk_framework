...
static DKFW_PROFILE g_profile_local;
static DKFW_PROFILE *g_profile = &g_profile_local;

void dkfw_profile_reset(int item_cnt)
{
    memset(g_profile, 0, sizeof(DKFW_PROFILE));
    g_profile->item_cnt = (item_cnt ? item_cnt : MAX_PROFILE_ITEM_NUM);
}

void dkfw_profile_reset_extmem(int item_cnt, void *mem)
{
    g_profile = mem;
    dkfw_profile_reset(item_cnt);
}
